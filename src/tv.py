...
while numFile < N:
    ...
    for line in f:
        v = line.split()
        # extract only the coordinate in xyz from column 2nd-5th
        values = map(float, v[2:5])
    ...
    while i < len(vector_raw)-1:
        ...
        while j < len(vector_raw):
            ...
            # 1.0 = global cut-off <-- You can flexibly change the distance of cutoff.
            if dist <= 0.8:
...
print(f"\n! ..... Total of Event = {total} .....")
print(f"! Normally Terminated on: {datetime.datetime.now()}")
print("Done")